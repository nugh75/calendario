...
from admin_utils import is_admin_logged_in, admin_interface, login_admin, logout_admin
...
def show_admin_management():
    ...
    from admin_utils import upload_excel_file
    ...
    with tab2:
        ...
        if uploaded_file is not None:
            # Aggiungiamo opzioni di analisi file
            col1, col2 = st.columns(2)
            with col1:
                import_btn = st.button("Importa Dati")
            with col2: 
                analyze_btn = st.button("🔍 Analizza File")
            ...
            if import_btn:
                with st.spinner("Elaborazione del file Excel in corso..."):
                    ...
                    if df_result is not None and not df_result.empty:
                        ...
                        # Proponi l'analisi dettagliata del file
                        if st.button("📊 Analizza problemi nel file"):
                            # Importiamo le utilità di validazione
                            from validation_utils import analyze_excel_upload, show_validation_results
                            with st.spinner("Analisi dettagliata del file in corso..."):
                                # Riavvolgi il file per poterlo leggere nuovamente
                                uploaded_file.seek(0)
                                validation_results = analyze_excel_upload(uploaded_file)
                                if validation_results:
                                    show_validation_results(validation_results, uploaded_file.name)
